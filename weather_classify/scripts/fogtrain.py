...
def patch(c = 5.81, g = 4.33):
    fog = np.load('fog2snow1.npy')

    l = fog.shape
    f_t = np.linspace(0, 0, l[0]/105)

    sunny = np.load('fog2snow3.npy')
    l = sunny.shape
    s_t = np.linspace(3, 3, l[0]/105)
    
    train = np.array(np.r_[fog[:2100], sunny[:2100]], dtype = np.float32)
    #train = np.array(np.r_[snow[:200],rain[:200]], dtype = np.float32)
    train.shape = -1 , 105 
    res = np.array(np.r_[f_t[:20], s_t[:20]], dtype = np.float32)

    test = np.array(np.r_[fog[2100:], sunny[2100:]], dtype = np.float32)
    test.shape = -1, 105 
    rres = np.array(np.r_[f_t[20:],s_t[20:]], dtype = np.float32)

    svm_params = dict( kernel_type = cv2.SVM_LINEAR, svm_type = cv2.SVM_C_SVC, C = c, gamma=g )

    svm = cv2.SVM()
    svm.train(train, res, params=svm_params)
    svm.save('svm_data_fog2sunny.dat')
